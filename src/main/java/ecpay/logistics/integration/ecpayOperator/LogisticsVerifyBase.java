...
public class LogisticsVerifyBase {
    ...
    public void verifyCreate(Object obj) {
        if (obj instanceof CreateCVSObj) {
            ...
            if (((CreateCVSObj) obj).getLogisticsSubType().equals("TCAT")
                    || ((CreateCVSObj) obj).getLogisticsSubType().equals("ECAN"))
                throw new EcpayException(
                        "LogisticsSubType cannot be " + ((CreateCVSObj) obj).getLogisticsSubType() + " when LogisticsType is CVS.");
                ...
                if (!((CreateCVSObj) obj).getCollectionAmount().isEmpty())
                    if (!((CreateCVSObj) obj).getGoodsAmount().equals(((CreateCVSObj) obj).getCollectionAmount()))
                        throw new EcpayException(
                                "GoodsAmount(" + ((CreateCVSObj) obj).getGoodsAmount() + ") cannot be equal to CollectionAmount("
                                        + ((CreateCVSObj) obj).getCollectionAmount() + ") when LogisticsSubType is UNIMARTC2C.");
            ...
            if (!((CreateHomeObj) obj).getLogisticsSubType().equals("TCAT")
                    && !((CreateHomeObj) obj).getLogisticsSubType().equals("ECAN"))
                throw new EcpayException("LogisticsSubType cannot be " + ((CreateHomeObj) obj).getLogisticsSubType()
                        + " when LogisticsType is Home.");
            // IsCollection 不能為Y
            ...
            if (((CreateHomeObj) obj).getReceiverPhone().isEmpty() && ((CreateHomeObj) obj).getReceiverCellPhone().isEmpty())
                throw new EcpayException(
                        "ReceiverPhone and ReceiverCellPhone cannot both be empty when LogisticsType is Home.");
            ...
            if (((CreateHomeObj) obj).getLogisticsSubType().equals("ECAN")) {
                ...
                if (!((CreateHomeObj) obj).getTemperature().equals("0001"))
                    throw new EcpayException(
                            "Temperature cannot be " + ((CreateHomeObj) obj).getTemperature() + " when LogisticsSubType is ECAN.");
                ...
                if (Integer.parseInt(((CreateHomeObj) obj).getPackageCount()) < 1
                        || Integer.parseInt(((CreateHomeObj) obj).getPackageCount()) > 999)
                ...
                // [LogisticsSubType]為TCAT => ScheduledDeliveryTime 不能為12:9~17, 13:9~12&17~20,
                // 23:13~20
                if (!((CreateHomeObj) obj).getScheduledDeliveryTime().isEmpty())
                    if (Integer.parseInt(((CreateHomeObj) obj).getScheduledDeliveryTime()) <= 1
                            && Integer.parseInt(((CreateHomeObj) obj).getScheduledDeliveryTime()) >= 23)
                        throw new EcpayException("ScheduledDeliveryTime cannot be "
                                + ((CreateHomeObj) obj).getScheduledDeliveryTime() + " when LogisticsSubType is TCAT.");
    ...
    public void verifyReturnHome(ReturnHomeObj obj) {
        ...
        // if [LogisticsSubType] isn't empty, it will check info_params values not
        // empty.
        String[] infoParams = {"SenderName", "SenderZipCode", "SenderAddress", "ReceiverName", "ReceiverZipCode",
                "ReceiverAddress"};
        if (obj.getLogisticsSubType().equals("TCAT") || obj.getLogisticsSubType().equals("ECAN")) {
            ...
            if (!Pattern.matches("^([\u4e00-\u9fa5]{1,5}|[a-zA-Z]{1,10})$", obj.getSenderName())
                    || !Pattern.matches("^([\u4e00-\u9fa5]{1,5}|[a-zA-Z]{1,10})$", obj.getReceiverName()))
                throw new EcpayException("SenderName or ReceiverName must be at most 5 Chinese or 10 alphabets.");
            if (!Pattern.matches("^.{7,61}$", obj.getSenderAddress())
                    || !Pattern.matches("^.{7,61}$", obj.getReceiverAddress()))
                throw new EcpayException("SenderAddress or ReceiverAddress must be 7-61 alphabets.");
            if (!Pattern.matches("^\\d{3,5}$", obj.getSenderZipCode())
                    || !Pattern.matches("^\\d{3,5}$", obj.getReceiverZipCode()))
                throw new EcpayException("SenderZipCode or ReceiverZipCode must be 3-5 numbers.");
            // [LogisitcsSubType]為TCAT or ECAN => SenderPhone and SenderCellPhone Can Not
            // Both be empty
            if (obj.getSenderPhone().isEmpty() && obj.getSenderCellPhone().isEmpty())
                throw new EcpayException("SenderPhone and SenderCellPhone cannot both be empty when LogisticsType is Home.");
            // [LogisitcsSubType]為TCAT or ECAN => ReceiverPhone and ReceiverCellPhone Can
            // Not Both be empty
            ...
            if (obj.getLogisticsSubType().equals("ECAN")) {
                ...
                if (!obj.getScheduledDeliveryTime().isEmpty())
                    if (Integer.parseInt(obj.getScheduledDeliveryTime()) <= 1
                            && Integer.parseInt(obj.getScheduledDeliveryTime()) >= 23)
                        throw new EcpayException("ScheduledDeliveryTime cannot be " + obj.getScheduledDeliveryTime()
                                + " when LogisticsSubType is TCAT.");
                // [LogisticsSubType]為TCAT => ScheduledPickupTime 不可為空
            ...
            for (String tmp : itemParams) {
                ...
                try {
                    ...
                    if (paramValue.contains("#"))
                        throw new EcpayException(
                                "Item info " + tmp + " contains # delimiter but there's only one item in GoodsName.");
                } catch (Exception e) {
                    throw new EcpayException(e.getMessage());
                }
            }
